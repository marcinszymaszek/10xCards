...
- `npx astro sync` must run before type checking if `.astro/types.d.ts` is missing or stale
- `npm run build` fails without `SUPABASE_URL` and `SUPABASE_KEY` — copy `.env.example` → `.env` and populate
- Pre-commit gate (Husky + lint-staged) blocks commits with lint errors; run `npm run lint:fix` first
- Vite is pinned to `^7.3.2` in `package.json` overrides — don't remove this pin when upgrading deps
...
| Command | Use |
|---------|-----|
| `npm run dev` | Astro dev server with Cloudflare workerd runtime |
| `npm run lint` | Full type-aware ESLint (slow — runs tsc) |
| `npm run lint:fix` | Auto-fix ESLint violations + Prettier format |
| `npm run build` | Production build (requires env vars — see Gotchas) |
| `npx astro sync` | Regenerate Astro types — run if imports break |
